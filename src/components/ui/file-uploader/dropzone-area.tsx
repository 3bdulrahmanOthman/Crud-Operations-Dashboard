...
import {
  ...
  validationPresets,
  type FileType,
} from "@/lib/validation/file-uploader";
...
export function DropzoneArea({
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    disabled:
      isUploading || disabled || uploadedUrls.length >= effectiveMaxFiles,
  ...
  return (
    ...
    >
      ...
      <div className="h-full flex flex-col items-center justify-center gap-1 text-xs text-muted-foreground">
        ...
        {customText?.fileTypes !== "empty" && (
          <p>
            {customText?.fileTypes ||
              `${
                fileType.charAt(0).toUpperCase() + fileType.slice(1)
              } files only (${preset.acceptedFileTypes.join(", ")})`}
          </p>
        )}
        {customText?.sizeInfo !== "empty" && (
          <p>
            {customText?.sizeInfo ||
              `Max ${effectiveMaxSize}MB per file • Up to ${effectiveMaxFiles} file${
